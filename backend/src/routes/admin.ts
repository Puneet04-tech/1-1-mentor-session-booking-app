    ...
    try {
      ...
      await query(
       ...
       SET is_suspended = $1,
           ...
           updated_at = NOW()
       WHERE id = $3`,
        [isSuspended, isSuspended ? reason.trim() : null, userId],
      );
...
router.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message: verified ? "Mentor verified" : "Mentor verification revoked",
      });
    } catch (error) {
